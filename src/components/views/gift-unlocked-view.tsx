...
interface GiftUnlockedViewProps {
  giftData: GiftData
}

export function GiftUnlockedView({ giftData }: GiftUnlockedViewProps) {
  ...
  const recipientName = giftData.recipient_name || "vous"
  const message = giftData.custom_message || "Un message plein d'amour..."
  
  // Use uploaded images or fallback to sample images
  const uploadedImageUrls = giftData.photos || []
  const sampleImages = [
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="fixed top-6 left-6 right-6 z-50 flex justify-between tracking-normal font-sans"
      >
        <Link href="/dashboard">
          <Button
            variant="outline"
            className="gap-2 rounded-xl border-white/30 bg-white/70 backdrop-blur-lg hover:bg-white/90 text-foreground"
          >
